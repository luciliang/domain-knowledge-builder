...
| **② 可审计 (Auditable)** | 每个知识节点标注 provenance（来源论文/章节/公式/页码）；results.tsv 记录每次评分变化；可追溯到生成时刻的输入与 commit | 抽查节点 frontmatter 的 sources 字段；节点是否可溯源到具体论文段落 |
...
_规范版本：darwin 2.0 | 适配项目：domain-knowledge-builder meta-skill | 质量门：B+ (80/100)_
